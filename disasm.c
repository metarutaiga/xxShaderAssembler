...
void DisassembleD3DSI(char* text, size_t size, size_t width, uint32_t version, uint32_t const tokens[8], size_t count)
{
    ...
    {
        // Constant
        if (j != 0 && (token & D3DSI_OPCODE_MASK) == D3DSIO_DEF)
        {
            float value;
...
//      case D3DSPR_OUTPUT:
        ...
        if (j != 0)
        {
            ...
            case D3DSPSM_X2NEG:     neg = "-";          break;
            case D3DSPSM_COMP:      neg = "1-";         break;
            }
            switch (token & D3DSP_SRCMOD_MASK)
            {
            case D3DSPSM_BIAS:
            case D3DSPSM_BIASNEG:   scale = "_bias";    break;
            case D3DSPSM_SIGN:
            case D3DSPSM_SIGNNEG:   scale = "_bx2";     break;
            case D3DSPSM_X2:
            case D3DSPSM_X2NEG:     scale = "_x2";      break;
            case D3DSPSM_DZ:        scale = "_dz";      break;
            case D3DSPSM_DW:        scale = "_dw";      break;
            }
        }
        if (type == D3DSPR_RASTOUT)
        {
            _PRINTF("%s%s%s", neg, reg, scale);
        }
        else
        {
            _PRINTF("%s%s%d%s", neg, reg, index, scale);
        }
        ...
        {
            _PRINTF("%s", ".");
            if ((token & D3DSP_WRITEMASK_0) == D3DSP_WRITEMASK_0) _PRINTF("%s", "x");
            if ((token & D3DSP_WRITEMASK_1) == D3DSP_WRITEMASK_1) _PRINTF("%s", "y");
            if ((token & D3DSP_WRITEMASK_2) == D3DSP_WRITEMASK_2) _PRINTF("%s", "z");
            if ((token & D3DSP_WRITEMASK_3) == D3DSP_WRITEMASK_3) _PRINTF("%s", "w");
        }
    }
}
//==============================================================================
